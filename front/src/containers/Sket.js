          ...
          <div className="login-content">
            ...
            <div className="login-button">
              <FacebookLogin
                ...
                callback={(res) => {
                  this.props.handleLogin('facebook', res)
                }}
              />
            </div>
...
const mapDispatchToProps = (dispatch) => {
  return {
    ...
    handleLogin: (social, user) => {
      dispatch(actions.handleLogin(social, user))
    },
    handleGuestLogin: () => {
      dispatch(actions.handleGuestLogin())
    },
    ...
    /* 대기 화면 핸들링 */
    handleCreateRoom: (roomInfo) => {
      dispatch(actions.createRoom(roomInfo))
    },
    handleQuickStart: () => {
      dispatch(actions.quickStart())
    },
    handleShowRanking: (userId) => {
      dispatch(actions.showRanking(userId))
    },
    handleEnterRoom: (roomId, userId) => {
      dispatch(actions.getRoomInfo(roomId, userId))
    },

    /* 게임 기능 핸들링 */
    handleRequestExitRoom: (roomId, userId) => {
      dispatch(actions.requestExitRoom(roomId, userId))
    },
    handleExitRoom: () => {
      dispatch(actions.exitRoom())
    },
    handleGetReady: (roomId, userId, isReady) => {
      dispatch(actions.getReady(roomId, userId, isReady))
    },
    handleStartGame: (roomId, userId) => {
      dispatch(actions.requestStartGame(roomId, userId))
    },
    handleStartTimer: () => {
    ...
    handleCanvasData: (data) => {
      dispatch(actions.drawCanvas(data))
    },
    handleChatData: (roomId, userId, restTime, msg) => {
      dispatch(actions.chatSend(roomId, userId, restTime, msg))
    },
    handlequizStart: (roomId) => {
      dispatch(actions.requestQuizStart(roomId))
    },
    handleTimeout: (roomId) => {
      dispatch(actions.noticeTimeout(roomId))
    },

    handleQuizModal: (bool) => {
      dispatch(actions.quizModal(bool))
    },
    handleQuizResultModal: (bool) => {
      dispatch(actions.quizResultModal(bool))
    },
    handleTimeoutModal: () => {
      dispatch(actions.timeoutModal())
    }
  };
}
